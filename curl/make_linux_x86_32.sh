...
cmake -DBUILD64=OFF -C "$rootdir/CMakeLists.txt" -D_OPENSSL_VERSION="$openssl_version" -DOPENSSL_INCLUDE_DIR="$rootdir/../openssl_build/target/include/$target" -DOPENSSL_CRYPTO_LIBRARY="$rootdir/../openssl_build/target/lib/$target/libcrypto.a" -DOPENSSL_SSL_LIBRARY="$rootdir/../openssl_build/target/lib/$target/libssl.a" -DBUILD_CURL_EXE=OFF -DBUILD_TESTING=OFF -DCURL_STATICLIB=ON -DCMAKE_INSTALL_PREFIX="$prefix" "$rootdir/$version"
